...
public class Parser : ParserBase, IParser
{
    ...
    public Parser()
    {
        ...
        _ast = new ASTNode(NodeType.Root, null);
    }
    ...
    private ASTNode Walk()
    {
        ...
        throw Error(CurrentToken, "Unexpected Token '{0}'", CurrentToken.Content);
    }
    ...
    private ASTNode ParseAssignment()
    {
        ...
        node.Children.Add(new ASTNode(NodeType.Identifier, CurrentToken.Content));
        Advance(2);
    ...
    private ASTNode ParseConstantAssignment()
    {
        ...
        var name = CurrentToken.Content;
        Advance();

        Consume(TokenType.Equals);

        Expect(TokenType.Number);
        var value = CurrentToken.Content;
    ...
    {
        if (functionParsers.TryGetValue(CurrentToken.Content, out ParseOperation value))
            return value();
        ...
        while (CurrentToken.Type != TokenType.RightBracket)
        {
            ...
            node.Children.Add(new ASTNode(CurrentToken.Type == TokenType.Number ? NodeType.Value : NodeType.Identifier, CurrentToken.Content));
        }
    ...
    private ASTNode ParseInput()
    {
        ...
        node.Children.Add(new ASTNode(NodeType.Value, CurrentToken.Content));
    ...
    {
        var variable = CurrentToken.Content;
        Advance();

        var sign = CurrentToken.Content;
        Advance(2);
    ...
    private ASTNode ParseArithmetic()
    {
        ...
        node.Children.Add(new ASTNode(NodeType.Identifier, CurrentToken.Content));
        Advance();
        node.Children.Add(new ASTNode(NodeType.Sign, CurrentToken.Content));
        Advance();
        node.Children.Add(Walk());
    ...
    {
        return CurrentToken.Type == TokenType.KeyWord && CurrentToken.Content == "if";
    }
    ...
    private ASTNode ParseConditionalStatement()
    {
        ...
        var blockStartLine = CurrentToken.Line;

        var block = new ASTNode(NodeType.Block);

        while (CurrentToken.Type != TokenType.RightCurlyBracket)
    ...
    {
        return PeekType(0, TokenType.KeyWord) && loopParsers.ContainsKey(CurrentToken.Content);
    }

    private ASTNode ParseLoop()
    {
        return loopParsers[CurrentToken.Content]();
    }
    ...
    private ASTNode ParseWhileLoop()
    {
        ...
        if (condition.Type != NodeType.Condition)
            throw Error(CurrentToken, "Invalid Condition provided");
    ...
    {
        return CurrentToken.Type == TokenType.KeyWord && CurrentToken.Content == "func";
    }
    ...
    {
        var prim = CurrentToken.Type switch
        {
            TokenType.Identifier => new ASTNode(NodeType.Identifier, CurrentToken.Content),
            TokenType.Number => new ASTNode(NodeType.Value, CurrentToken.Content),
        };
